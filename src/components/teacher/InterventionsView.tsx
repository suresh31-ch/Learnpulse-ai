...
export const InterventionsView: React.FC = () => {
  ...
  const [blueprint, setBlueprint] = useState({
    title: "Targeted Sign-Rule Recovery Session",
    topic: "Quadratic Equations",
    affectedCount: 19,
    description: "Targeted specifically to the 19 students confusing negative constant multiplication in Δ = b² - 4ac.",
    timeline: [
      {
        phase: "Phase 1",
        time: "00:00 – 03:00",
        title: "Concept Recap",
        desc: "Focus specifically on why -4ac becomes addition whenever c is negative. Use high-contrast color notation on chalkboard."
      },
      {
        phase: "Phase 2",
        time: "03:00 – 07:00",
        title: "Worked Example",
        desc: "Solve 2x² - 3x - 5 = 0 step-by-step together, highlighting the transformation from 9 - (-40) to 9 + 40 = 49."
      },
      {
        phase: "Phase 3",
        time: "07:00 – 12:00",
        title: "Guided Peer Practice",
        desc: "Students solve 2 paired questions on their tablets with real-time confidence calibration tags."
      },
      {
        phase: "Exit Check",
        time: "12:00 – 15:00",
        title: "Mastery Verification",
        desc: "2 quick exit questions auto-scored to measure verified learning recovery before lecture concludes."
      }
    ]
  });
  ...
  const handleGenerateLivePlan = async () => {
    setLoadingAI(true);
    try {
      const res = await fetch('/api/ai/generate-intervention', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          topic: blueprint.topic,
          gapDescription: "Sign error in discriminant calculation (multiplying negative constant c)",
          studentsCount: blueprint.affectedCount
        })
      });
      const data = await res.json();
      const plan = data.intervention || data.data;
      if (plan) {
        setIsLiveAI(data.isLiveAI);
        setBlueprint({
          title: plan.title || blueprint.title,
          topic: blueprint.topic,
          affectedCount: blueprint.affectedCount,
          description: plan.objective || blueprint.description,
          timeline: (plan.timeline || []).map((t: any, idx: number) => ({
            phase: idx === 3 ? "Exit Check" : `Phase ${idx + 1}`,
            time: t.minutes || "00:00 - 04:00",
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-md space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">Class 10A • {blueprint.topic}</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <button
              id="btn-generate-ai-intervention"
              onClick={handleGenerateLivePlan}
              disabled={loadingAI}
              className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-all border border-slate-700 flex items-center gap-1.5"
            >
              {loadingAI ? (
                <>
            ...
            {!published ? (
              ...
              <div className="px-4 py-2 bg-emerald-600 text-white text-xs font-bold rounded-xl flex items-center gap-1.5">
                ...
                <span>Published to {blueprint.affectedCount} Students!</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {blueprint.timeline.map((step, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700/80 space-y-2">
              <div className="flex items-center justify-between">
                <span className={`text-xs font-mono font-bold ${idx === 3 ? 'text-emerald-400' : 'text-indigo-300'}`}>
                  {step.time}
                </span>
