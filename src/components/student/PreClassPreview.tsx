...
import { computeReadinessPercentage } from '../../lib/gapDetection';
import { persistReadinessCheck } from '../../lib/studentPersistence';
import { DemoDataBadge } from '../shared/StatusBadge';
...
export const PreClassPreview: React.FC<PreClassPreviewProps> = ({
  ...
  const prerequisites = useMemo(() => {
    return data.prerequisites.map((p) => {
      const live = studentData.conceptMastery.find(
        (c) =>
          c.concept_name.toLowerCase().includes(p.name.toLowerCase().slice(0, 12)) ||
          p.name.toLowerCase().includes(c.concept_name.toLowerCase().slice(0, 12)),
      );
      const masteryPercentage = live?.mastery_score ?? p.masteryPercentage;
      const status: typeof p.status =
        masteryPercentage >= 75 ? 'passed' : masteryPercentage >= 60 ? 'warning' : 'gap';
      return { ...p, masteryPercentage, status };
    });
  }, [data.prerequisites, studentData.conceptMastery]);
  ...
  const calculateScore = calculateQuizAccuracy;

  const quizAccuracy = submitted ? calculateQuizAccuracy() : null;
  const dynamicReadiness = computeReadinessPercentage(
    prerequisites.map((p) => p.masteryPercentage),
    quizAccuracy,
  );

  const handleSubmitQuiz = async () => {
    setSubmitted(true);
    const score = calculateQuizAccuracy();
    ...
    const missed = data.quickQuestions
      .filter((q) => userAnswers[q.id] !== q.correctIndex)
      .map((q) => q.conceptChecked);
    if (user?.id) {
      const ok = await persistReadinessCheck({
        studentId: user.id,
        topicId: studentData.upcomingSession?.topicId || data.topicId,
        quizAccuracy: score,
        readiness,
      ...
      setPersistNote(
        ok
          ? 'Readiness saved to your learning events (preclass_readiness).'
          : 'Readiness calculated locally. Could not persist — check RLS or table access.',
      );
      studentData.refresh();
    } else {
      setPersistNote('Sign in to persist readiness to Supabase learning events.');
    }
  };
    ...
    <div id="pre-class-preview-view" className="space-y-6">
          ...
          <div className="bg-slate-800/80 backdrop-blur-md rounded-2xl p-4 border border-slate-700/80 text-center min-w-[150px]">
            ...
            <div className="text-3xl font-extrabold text-white mt-1">
              ...
              <span className="text-[11px] text-slate-400">
                {submitted ? '55% quiz + 45% prerequisite mastery' : 'From recorded prerequisite mastery'}
              </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200">
        ...
        >
          ...
          Prerequisite Checks ({prerequisites.length})
        </button>
        <button
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {isPassed ? 'Ready' : isWarn ? 'Watch' : 'Detected Gap'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">
                      {isPassed
